...
const getPlaceholderPosition = (pattern) => {
  const index = pattern.indexOf(PLACEHOLDER_SYMBOL)
  return index === -1
    ? null
    : {
      from: index,
      to: index + PLACEHOLDER_SYMBOL.length - pattern.length
    }
}

exports.placeholders = (directory, rules) => {
  return rules.reduce((accumulator, rule) => {
    const position = getPlaceholderPosition(rule.antecedent.pattern)
    if (!position) {
      return accumulator.concat(rule)
    }

    if (rule.antecedent.negative) {
      throw new errors.InvalidConfigurationLine(rule.filename, rule.line,
        'placeholder antecedents must not be negative patterns')
    }

    if (rule.antecedent.pattern.indexOf(PLACEHOLDER_SYMBOL) !==
    ...
    return accumulator.concat(results.map((result) => {
      const value = result.slice(position.from, position.to)
      return {
        antecedent: {
